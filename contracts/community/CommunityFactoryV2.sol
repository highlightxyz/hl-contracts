...
contract CommunityFactoryV2 is Ownable {
    ...
    /**
     * @dev Default global token managers
     */
    address[] public defaultRegisteredTokenManagers;
    ...
    /**
     * @notice Emitted when default registered token managers are set
     * @param _defaultRegisteredTokenManagers New set of default registered token managers
     */
    event SetDefaultRegisteredTokenManagers(address[] _defaultRegisteredTokenManagers);

    /**
     * @dev Sets up factory
     * @param _initialOwner Initial owner of factory
     * @param _splitMain System royalty manager
     * @param _proxyAdmin PermissionsRegistry and SplitMain proxy admin
     * @param _permissionsRegistry Registry of system permissions / variables
     * @param _communityTrustedForwarder Trusted meta tx relayer for communities
     * @param _defaultRegisteredTokenManagers The default token managers that should be registered on each community
     */
    constructor(
        ...
        address[] memory _defaultRegisteredTokenManagers
    ) public {
        splitMain = _splitMain;
        proxyAdmin = _proxyAdmin;
        permissionsRegistry = _permissionsRegistry;
        communityTrustedForwarder = _communityTrustedForwarder;
        defaultRegisteredTokenManagers = _defaultRegisteredTokenManagers;
        basicTokenManagerImplementation = address(new BasicTokenManagerUpgradeable2());
        _transferOwnership(_initialOwner);
    }

    /**
     * @dev Set the global default registered token managers
     * @param _defaultRegisteredTokenManagers New default registered token managers
     */
    function setDefaultRegisteredTokenManagers(address[] calldata _defaultRegisteredTokenManagers) external onlyOwner {
        defaultRegisteredTokenManagers = _defaultRegisteredTokenManagers;
        emit SetDefaultRegisteredTokenManagers(_defaultRegisteredTokenManagers);
    }
    ...
    /**
     * @dev Deploys community proxy, and registers default global token managers on community
     * @param _beacon Beacon the community proxy will point to
     * @param _creatorAdmin Creator admin of community
     * @param _defaultAdmin Default admin of community
     * @param _owner "Owner" of community, used to administrate collections on external marketplaces
     ...
     * @param userDefinedNonce User defined nonce used to bypass Create2 error
     */
    function setupCommunityWithGlobalDefaultTokenManagers(
        address _beacon,
        address _creatorAdmin,
        address _defaultAdmin,
        address _owner,
        ...
        // cache
        address[] memory defaultRegisteredTokenManagersTemp = defaultRegisteredTokenManagers;
        uint256 defaultTokenManagersLength = defaultRegisteredTokenManagersTemp.length;

        for (uint256 i = 0; i < defaultTokenManagersLength; i++) {
            ICommunity(newBasicCommunity).registerTokenManager(defaultRegisteredTokenManagersTemp[i]);
        }

        IAccessControlUpgradeable(newBasicCommunity).revokeRole(keccak256("PLATFORM_ROLE"), address(this));
        ICommunityAdmin(newBasicCommunity).swapDefaultAdmin(_defaultAdmin);

        return newBasicCommunity;
    }
    ...
    /**
     * @dev Grab the default registered token managers
     * @return The default registered token managers
     */
    function getDefaultRegisteredTokenManagers() external view returns (address[] memory) {
        return defaultRegisteredTokenManagers;
    }
